            ...
            <div key={standard.id} className="bg-white rounded-[40px] overflow-hidden shadow-2xl border border-white/10">
              ...
              <div className="p-10 text-slate-900 grid lg:grid-cols-3 gap-10">
                ...
                <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-inner">
                  <h4 className="text-[10px] font-black uppercase text-indigo-600 tracking-widest mb-6">Probe Questions ({level})</h4>
                  <div className="space-y-6">
                    {standard.questions[level].map((q, i) => (
                      <p key={i} className="text-xs font-semibold italic text-slate-700 leading-relaxed border-l-3 border-indigo-200 pl-4">{q}</p>
                    ))}
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {(['Strong Evidence', 'Good Evidence', 'Limited Evidence', 'No Evidence'] as Rating[]).map(r => (
                        <button
                          ...
                          className={`text-left px-5 py-4 rounded-2xl border-2 font-black text-[11px] uppercase tracking-wider transition-all ${
                            ratings[standard.id] === r 
        ...
        <div className="mt-24 bg-white rounded-[40px] p-12 text-slate-900 border-8 border-[#d4af37] shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
                ...
                <div className="grid gap-4">
                  {(['Decline', 'Progress to next stage', 'Offer'] as EvaluationOutcome[]).map(o => (
                    <button
                      ...
                      className={`text-left p-5 rounded-2xl border-2 font-black uppercase text-xs tracking-widest transition-all bg-white ${
                        finalOutcome === o 
            ...
            <button 
              ...
              className={`w-full mt-12 py-8 rounded-3xl font-black uppercase tracking-[0.3em] text-lg shadow-2xl transition-all ${
                isFormComplete ? 'bg-[#002b49] text-white hover:scale-[1.01] hover:bg-slate-900 shadow-[#002b49]/20' : 'bg-slate-100 text-slate-300 cursor-not-allowed shadow-none'
              }`}
            >
              Complete Evaluation
            </button>
