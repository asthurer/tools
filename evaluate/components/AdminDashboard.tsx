      ...
      <div className="flex gap-2">
        <button 
          ...
          className={`p-4 rounded-2xl border-2 transition-all ${
            currentPage === 1 ? 'opacity-30 cursor-not-allowed border-slate-100' : 'hover:bg-slate-50 border-slate-200 active:scale-95'
          }`}
        >
        ...
        <button 
          ...
          className={`p-4 rounded-2xl border-2 transition-all ${
            currentPage === totalPages || totalPages === 0 ? 'opacity-30 cursor-not-allowed border-slate-100' : 'hover:bg-slate-50 border-slate-200 active:scale-95'
          }`}
        >
        ...
        <div className="bg-white w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-12 no-scrollbar">
            <div className="grid lg:grid-cols-2 gap-12">
              <section>
                <h3 className="text-xs font-black uppercase text-slate-400 tracking-[0.2em] mb-6 border-b pb-2">Technical Assessment Overview</h3>
                <div className="bg-slate-50 rounded-[32px] p-10 border border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-8 shadow-sm">
                  <div className="flex flex-col items-center sm:items-start">
                    <div className="text-[44px] font-black text-[#002b49] leading-none mb-1">{selectedProfile.scorePercent.toFixed(2)}%</div>
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Composite Score</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Correct</span>
                      <span className="text-lg font-black text-green-600 leading-none">{selectedProfile.correctCount}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Missed</span>
                      <span className="text-lg font-black text-red-600 leading-none">{selectedProfile.missedCount}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Time</span>
                      <span className="text-sm font-black text-slate-900 leading-none">
                        {Math.floor(selectedProfile.totalTimeTakenSec / 60)}M {selectedProfile.totalTimeTakenSec % 60}S
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Avg Speed</span>
              ...
              <section>
                <h3 className="text-xs font-black uppercase text-slate-400 tracking-[0.2em] mb-6 border-b pb-2">Leadership Evidence</h3>
                {evaluations.find(e => e.candidateEmail === selectedProfile.candidateEmail) ? (
                  <div className="grid grid-cols-1 gap-3">
                    {LEADERSHIP_STANDARDS.map(s => {
                      ...
                      return (
                        <div key={s.id} className="flex justify-between items-center p-4 border border-slate-100 rounded-xl bg-white shadow-sm hover:shadow-md transition-all">
                          <div className="text-[10px] font-black uppercase text-[#002b49] tracking-tight">{s.title}</div>
                          <div className={`px-2 py-0.5 rounded text-[9px] font-black border uppercase ${getRatingColor(evaluation.ratings[s.id] as Rating)}`}>
                  ...
                  <div className="py-12 text-center text-slate-300 font-black uppercase text-[10px] border-2 border-dashed border-slate-100 rounded-2xl">No Interview Conducted</div>
                )}
              </section>
            </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex border-b border-slate-200 mb-10 gap-8 overflow-x-auto no-scrollbar">
        {(['leaderboard', 'assessments', 'questions', 'config'] as const).map(tab => (
          <button
            ...
            className={`px-2 py-4 font-black text-[10px] uppercase tracking-[0.2em] transition-all border-b-2 whitespace-nowrap ${
              activeTab === tab ? 'border-[#002b49] text-[#002b49]' : 'border-transparent text-slate-300 hover:text-slate-400'
            }`}
          >
      ...
      {loading && !syncMessage && activeTab !== 'questions' ? (
        ...
        <div className="pb-24">
          {syncMessage && (
            <div className={`mb-8 p-6 rounded-3xl text-center font-black uppercase text-[10px] tracking-widest border-2 shadow-sm transition-all animate-bounce ${
              syncMessage.type === 'success' ? 'bg-green-50 border-green-100 text-green-600' :
                ...
                return (
                  <div key={r.attemptId} className="bg-white rounded-[48px] p-12 border border-slate-100 shadow-xl flex flex-col justify-between hover:shadow-2xl transition-all group">
                    ...
                    <button 
                      ...
                      className={`w-full py-6 rounded-3xl font-black uppercase text-[11px] tracking-widest transition-all ${
                        isDone ? 'bg-slate-100 text-slate-400 hover:bg-slate-200' : 'bg-[#002b49] text-[#d4af37] shadow-xl hover:bg-[#002b49]/90'
                      }`}
                    >
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {(['A', 'B', 'C', 'D'] as OptionKey[]).map((key) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className={`w-10 h-10 flex items-center justify-center rounded-xl font-black text-sm mr-4 shrink-0 shadow-sm ${
                                        isCorrect ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-400'
                                      }`}>
                                        {key}
                                      </div>
